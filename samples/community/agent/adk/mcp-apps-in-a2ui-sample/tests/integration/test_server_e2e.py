...
A2A_RPC_URL = BASE_URL + "/a2a/mcp_apps_in_a2ui_sample/"
...
def start_server() -> subprocess.Popen[str]:
    ...
    command = [
        ...
        "-m",
        "uvicorn",
        "mcp_apps_in_a2ui_sample.fast_api_app:app",
        "--host",
        "0.0.0.0",
        "--port",
        "8000",
...
def test_adk_run_sse(server_fixture: subprocess.Popen[str]) -> None:
    ...
    session_response = requests.post(
        f"{BASE_URL}/apps/mcp_apps_in_a2ui_sample/users/{user_id}/sessions",
        headers=HEADERS,
        json=session_data,
        timeout=60,
    )
    ...
    data = {
        "app_name": "mcp_apps_in_a2ui_sample",
        "user_id": user_id,
